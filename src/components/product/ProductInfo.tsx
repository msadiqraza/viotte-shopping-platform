    ...
    <div className="flex flex-col space-y-4">
      ...
      <div className="flex items-center space-x-2">
        <div className="flex items-center">
          {[...Array(5)].map((_, i) => (
            <Star key={i} size={18} className={i < Math.round(product.rating) ? "text-orange-400 fill-orange-400" : "text-slate-300"} />
          ))}
        </div>
            ...
            {product.sizes.map((size) => (
              <button
                ...
                className={`px-4 py-1.5 border rounded-md text-sm transition-colors ${
                  selectedSize === size ? "bg-green-700 text-white border-green-700" : "bg-white text-slate-700 border-slate-300 hover:border-green-500"
                }`}
              >
                {size}
              </button>
            ...
            {product.colors.map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-full border-2 transition-all duration-150 ${
                  selectedColor === color.name ? "ring-2 ring-offset-1 ring-green-700" : "hover:opacity-80"
                }`}
                style={{ backgroundColor: color.hex, borderColor: selectedColor === color.name ? color.hex : "#d1d5db" }}
              />
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        <div className="flex items-center border border-slate-300 rounded-md w-fit">
          ...
          <input type="number" value={quantity} readOnly className="w-12 text-center border-none focus:ring-0 text-sm" />
          <button
            onClick={() => handleQuantityChange(1)}
        ...
        {product.stock !== undefined && <p className="text-xs text-slate-500 mt-1">{product.stock > 0 ? `${product.stock} items in stock` : "Out of stock"}</p>}
      </div>
      <div className="flex flex-col sm:flex-row gap-3 pt-2">
        ...
        >
          <CartIconProdInfo size={18} className="mr-2" /> Add to Cart
        </button>
        <button
      ...
      <div className="flex items-center space-x-4 pt-2">
        <button onClick={() => console.log("Chat with seller")} className="text-sm text-green-600 hover:underline flex items-center">
          <MessageSquare size={16} className="mr-1.5" /> Chat with Seller
        </button>
        <button onClick={() => onWishlist(product.id)} className="text-sm text-slate-600 hover:text-orange-500 flex items-center">
          <HeartIcon size={16} className="mr-1.5" /> Add to Wishlist
        </button>
      </div>
    </div>
  );
