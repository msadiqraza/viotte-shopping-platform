...
export const Navbar: React.FC<NavbarProps> = ({ onNavigate }) => {
	...
	if (user) {
		mobileNavLinks.push({ name: "Logout", page: "logout", icon: LogOut, separatorBefore: true });
	}
				...
				<div className="flex items-center justify-between h-16">
					...
					<div className="hidden md:flex flex-1 max-w-xl mx-4">
						<form onSubmit={handleSearchSubmit} className="relative w-full">
							...
							/>
							<button type="submit" className="absolute inset-y-0 left-0 pl-3 flex items-center cursor-pointer">
					...
					<div className="hidden md:flex items-center space-x-5 lg:space-x-6">
						{isAdmin && (
							<a
								...
								title="Admin Panel"
							>
								<ShieldCheckIcon size={24} /> <span className="text-xs mt-0.5 leading-tight">Admin</span>
							</a>
						)}
						<a
							href="#"
						...
						{user && (
							<a
								...
								title="Logout"
							>
								<LogOut size={24} /> <span className="text-xs mt-0.5 leading-tight">Logout</span>
							</a>
						)}
					</div>
						...
						link.dropdown && link.name === "Categories" ? (
							<div key={link.page} className="relative" ref={categoriesDropdownRef}>
								...
								>
									...
									<ChevronDown
										size={16}
										className={`ml-1 transition-transform duration-200 ${isCategoriesDropdownOpen ? "rotate-180" : ""}`}
									/>
								</button>
								{isCategoriesDropdownOpen && (
									<div
							...
							<React.Fragment key={link.page + index}>
								...
								{link.dropdown && link.name === "Categories" ? (
									...
									<div>
										...
										>
											{link.name}{" "}
											<ChevronDown size={16} className={`${isCategoriesDropdownOpen ? "rotate-180" : ""}`} />
										</button>
										{isCategoriesDropdownOpen && (
