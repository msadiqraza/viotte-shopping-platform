    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col group transition-all duration-300 hover:shadow-xl h-full">
      ...
      >
        <img
          ...
          onError={(e) =>
            (e.currentTarget.src = "https://placehold.co/400x300/e0e0e0/757575?text=No+Image")
          }
        />
      </a>
      <div className="p-4 flex flex-col flex-grow">
        ...
        <button
          className="mt-auto w-full max-w-[200px] bg-orange-400 hover:bg-orange-500 text-white text-sm font-medium py-2 px-3 rounded-md transition-colors flex items-center justify-center"
          onClick={() => {
            onAddToCart(product.id, 1, product.price, product.name, product.imageUrl);
            setIsAddedToCart(true);
          }}
        >
          <CartIcon size={18} className="mr-2" /> {isAddedToCart ? "Added" : "Add to Cart"}
        </button>
      </div>
    </div>
  );
